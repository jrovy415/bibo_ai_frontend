...
export default function Login() {
  ...
  const [currentStepWordIndex, setCurrentStepWordIndex] = useState(-1);
    ...
    speakText(text, {
      ...
      onBoundary: (charIndex) => {
        ...
        for (let i = 0; i < words.length; i++) {
          ...
          if (charIndex < cumulative) { setTutorialWordIndex(i); break; }
        }
      },
      onEnd: () => { setTutorialWordIndex(-1); setIsTutorialSpeaking(false); },
    });
  };

  useEffect(() => { setTimeout(() => setShowTutorial(true), 800); }, []);

  useEffect(() => {
    if (showTutorial) speakTutorialStep(tutorialStep);
    else { window.speechSynthesis.cancel(); setTutorialWordIndex(-1); setIsTutorialSpeaking(false); }
  ...
  useEffect(() => { if (!showIntroText) setHighlightedWordIndex(-1); }, [showIntroText]);
  ...
  return (
    <div style={{ margin: 0, width: '100vw', height: '100vh', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', fontFamily: "'Nunito', sans-serif" }}>
      ...
      {/* ── LAYOUT: side-by-side when tutorial is open ── */}
      <div style={{
        ...
        {showTutorial && (
          <div style={{
            ...
            {/* Pointing arrow toward the form */}
            <div style={{
              position: 'absolute',
              right: '-18px',
              top: '50%',
            ...
            <div style={{ fontSize: '3.5rem', animation: 'mascotBob 1.8s ease-in-out infinite', marginBottom: '0.2rem', display: 'inline-block' }}>
              {currentTutorial.emoji}
            </div>

            <h2 style={{ margin: '0 0 0.5rem', fontFamily: "'Fredoka One', cursive", fontSize: '1.45rem', color: 'white', textShadow: '0 3px 10px rgba(0,0,0,0.35)' }}>
              {currentTutorial.title}
            </h2>

            <p style={{ margin: '0 0 0.9rem', fontSize: '1rem', color: 'white', lineHeight: 1.65, fontWeight: 700 }}>
            ...
            {/* Replay button */}
            <button
              onClick={() => speakTutorialStep(tutorialStep)}
              style={{
                background: 'rgba(255,255,255,0.22)', border: '2px solid rgba(255,255,255,0.55)',
                borderRadius: '50px', padding: '0.35rem 1.3rem', color: 'white', fontSize: '0.88rem',
                fontWeight: 800, cursor: 'pointer', marginBottom: '1rem', fontFamily: "'Nunito', sans-serif",
                display: 'block', margin: '0 auto 1rem',
                animation: isTutorialSpeaking ? 'speakingPulse 0.8s ease-in-out infinite' : 'none',
            ...
            {/* Progress dots */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '1.1rem' }}>
              {tutorialSteps.map((_, i) => (
                <div key={i} style={{
                  width: i === tutorialStep ? '28px' : '10px', height: '10px',
                  borderRadius: '5px',
                  background: i === tutorialStep ? 'white' : 'rgba(255,255,255,0.35)',
            ...
            <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'center' }}>
              <button onClick={handleSkipTutorial}
                style={{ background: 'rgba(255,255,255,0.18)', color: 'white', border: '2px solid rgba(255,255,255,0.45)', borderRadius: '50px', padding: '0.6rem 1.3rem', fontSize: '0.95rem', fontWeight: 800, cursor: 'pointer', fontFamily: "'Nunito', sans-serif", transition: 'all 0.2s' }}
              ...
              <button onClick={handleTutorialNext}
                style={{ background: 'white', color: currentTutorial.color, border: 'none', borderRadius: '50px', padding: '0.6rem 1.6rem', fontSize: '1rem', fontWeight: 900, cursor: 'pointer', fontFamily: "'Fredoka One', cursive", boxShadow: '0 6px 20px rgba(0,0,0,0.25)', transition: 'all 0.25s cubic-bezier(.34,1.56,.64,1)' }}
                onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.1) translateY(-2px)'; e.currentTarget.style.boxShadow = '0 10px 30px rgba(0,0,0,0.3)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(0,0,0,0.25)'; }}
              >
                {tutorialStep < tutorialSteps.length - 1 ? 'Next →' : "Let's Go! 🚀"}
              </button>
            </div>
          </div>
        )}
              ...
              {activeTab === 'Student' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      {['Kinder', 'Grade 1'].map((grade) => {
                        ...
                        return (
                          <button key={grade} onClick={() => setGradeLevel(grade)} style={{
                            flex: 1, padding: '0.8rem 0.5rem', borderRadius: '16px', border: isActive ? '2px solid rgba(255,255,255,0.35)' : '1.5px solid rgba(255,255,255,0.1)', cursor: 'pointer',
                      ...
                      {['1', '2', '3', '4'].map((sec) => {
                        ...
                        return (
                          <button key={sec} onClick={() => setSection(sec)} style={{
                            flex: 1, padding: '0.85rem 0.4rem', borderRadius: '16px', border: isActive ? '2px solid rgba(255,255,255,0.35)' : '1.5px solid rgba(255,255,255,0.1)', cursor: 'pointer',
                  ...
                  <button
                    ...
                    style={{
                      padding: '1.15rem', borderRadius: '20px', border: isHighlighted('login') ? '3px solid white' : '2px solid rgba(255,150,0,0.4)', cursor: 'pointer',
