...
export default function Login() {
  ...
  const speakTutorialStep = useCallback((step, afterSpeak) => {
    ...
    speakText(text, {
      ...
      onEnd: () => { setTutorialWordIndex(-1); setIsTutorialSpeaking(false); afterSpeak?.(); },
    });
  }, []);
  ...
  const handleSkipTutorial = () => {
    ...
    setIsBetweenSteps(false); setIsTutorialSpeaking(false); setTutorialWordIndex(-1);
  };
  ...
  useEffect(() => { const t = setTimeout(() => setShowTutorial(true), 800); return () => clearTimeout(t); }, []);

  useEffect(() => {
    if (!showTutorial) {
      window.speechSynthesis?.cancel();
      clearTimeout(autoAdvanceRef.current);
      setTutorialWordIndex(-1); setIsTutorialSpeaking(false); setIsBetweenSteps(false);
      return;
    }
    speakTutorialStep(tutorialStep, () => advanceAfterPause(tutorialStep));
  ...
  // ── UPDATED: handle locked response ──────────────────────────────────────
  const handleStudentSubmit = async (e) => {
    ...
    try {
      ...
      // If student account is locked — redirect to locked screen
      if (locked) {
        navigate('/locked', { state: { nickname: student?.nickname } });
        return;
      }
    ...
    return (
      <div style={loadingStyle}>
        <div style={{ textAlign: 'center', color: 'white' }}>
          ...
          <style>{`@import url('https://fonts.googleapis.com/css2?family=Fredoka+One&family=Nunito:wght@700;900&display=swap'); @keyframes mascotBob { 0%,100%{transform:translateY(0) rotate(-3deg)} 50%{transform:translateY(-12px) rotate(3deg)} }`}</style>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ margin: 0, width: '100vw', height: '100vh', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ position: 'absolute', top: '18px', left: '50%', transform: 'translateX(-50%)', zIndex: 10, textAlign: 'center', pointerEvents: 'none', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ fontFamily: "'Fredoka One', cursive", fontSize: '2.2rem', color: 'white', textShadow: '0 0 40px #a78bfa, 0 0 80px #7c3aed55, 0 4px 8px rgba(0,0,0,0.6)', letterSpacing: '3px', lineHeight: 1 }}>BiboAI</div>
        <div style={{ fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: '0.75rem', color: '#c4b5fd', letterSpacing: '4px', textTransform: 'uppercase', marginTop: '2px' }}>✦ Learning Adventure ✦</div>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 5, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: showTutorial ? '1.5rem' : '0', width: '100%', maxWidth: '960px', padding: '0 1rem', marginTop: '85px', transition: 'gap 0.4s ease' }}>

        {/* Tutorial panel */}
        {showTutorial && (
          <div style={{ width: '340px', flexShrink: 0, background: `linear-gradient(145deg, ${currentTutorial.color}ee, ${currentTutorial.color}aa)`, borderRadius: '28px', padding: '1.6rem 1.8rem', boxShadow: `0 0 60px ${currentTutorial.color}55, 0 20px 60px rgba(0,0,0,0.5)`, border: '3px solid rgba(255,255,255,0.5)', animation: 'tutorialSlideIn 0.45s cubic-bezier(.36,.07,.19,.97)', textAlign: 'center', position: 'relative' }}>
            ...
            <div style={{ fontSize: '3.5rem', animation: isBetweenSteps ? 'twinkle 0.4s ease-in-out infinite' : 'mascotBob 1.8s ease-in-out infinite', marginBottom: '0.2rem', display: 'inline-block' }}>{currentTutorial.emoji}</div>
            <h2 style={{ margin: '0 0 0.5rem', fontFamily: "'Fredoka One', cursive", fontSize: '1.45rem', color: 'white', textShadow: '0 3px 10px rgba(0,0,0,0.35)', animation: 'stepFadeIn 0.35s ease both' }}>{currentTutorial.title}</h2>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'center' }}>
              <button onClick={handleReplay} disabled={isBetweenSteps} style={{ background: 'rgba(255,255,255,0.22)', border: '2px solid rgba(255,255,255,0.55)', borderRadius: '50px', padding: '0.5rem 1.2rem', color: 'white', fontSize: '0.88rem', fontWeight: 800, cursor: isBetweenSteps ? 'not-allowed' : 'pointer', fontFamily: "'Nunito', sans-serif", opacity: isBetweenSteps ? 0.45 : 1, animation: isTutorialSpeaking && !isBetweenSteps ? 'speakingPulse 0.8s ease-in-out infinite' : 'none' }}>{isTutorialSpeaking ? '🔊 Speaking...' : '🔁 Replay'}</button>
              <button onClick={handleSkipTutorial} style={{ background: 'rgba(255,255,255,0.18)', color: 'white', border: '2px solid rgba(255,255,255,0.45)', borderRadius: '50px', padding: '0.5rem 1.2rem', fontSize: '0.95rem', fontWeight: 800, cursor: 'pointer', fontFamily: "'Nunito', sans-serif" }}>Skip ✕</button>
            </div>
          </div>
        )}

        {/* Main login card */}
        <div style={{ width: '460px', maxWidth: '95vw', flexShrink: 0, animation: cardShake ? 'shake 0.4s ease' : 'cardFloat 4s ease-in-out infinite', position: 'relative' }}>
          ...
          <div style={{ background: 'rgba(8,8,35,0.92)', backdropFilter: 'blur(28px)', borderRadius: '32px', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 30px 90px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.1)', overflow: 'hidden' }}>
            <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              {['Student', 'Teacher'].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} style={{ flex: 1, padding: '1.1rem 1rem', border: 'none', cursor: 'pointer', fontFamily: "'Fredoka One', cursive", fontSize: '1.1rem', background: activeTab === tab ? 'rgba(167,139,250,0.15)' : 'transparent', color: activeTab === tab ? 'white' : 'rgba(255,255,255,0.4)', borderBottom: activeTab === tab ? '3px solid #a78bfa' : '3px solid transparent', transition: 'all 0.3s ease', letterSpacing: '0.5px' }}>{tab === 'Student' ? '🎒 Student' : '👩‍🏫 Teacher'}</button>
              ))}
            </div>
            <div style={{ padding: '1.75rem 1.75rem 2rem' }}>
              {activeTab === 'Student' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {/* Student Number */}
                  <div style={{ borderRadius: '20px', padding: '1rem 1.2rem', background: isHighlighted('nickname') ? 'rgba(104,211,145,0.15)' : 'rgba(255,255,255,0.04)', border: isHighlighted('nickname') ? '2px solid #68d391' : '1.5px solid rgba(255,255,255,0.08)', boxShadow: isHighlighted('nickname') ? '0 0 30px rgba(104,211,145,0.45),inset 0 0 20px rgba(104,211,145,0.08)' : 'none', transition: 'all 0.3s ease', animation: isHighlighted('nickname') ? 'glowPulse 1.5s ease-in-out infinite' : 'none' }}>
                    <label style={{ display: 'block', fontFamily: "'Fredoka One', cursive", fontSize: '0.95rem', color: '#a78bfa', marginBottom: '0.6rem', letterSpacing: '0.5px' }}>✏️ Student Number</label>
                    <input type="text" value={nickname} onChange={(e) => { if (/^[0-9-]*$/.test(e.target.value)) setNickname(e.target.value); }} placeholder="Example: 24-4824" inputMode="numeric" style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: '14px', border: '1.5px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.07)', color: 'white', fontSize: '1.05rem', fontFamily: "'Nunito', sans-serif", fontWeight: 700, outline: 'none' }} />
                    {studentError && <div style={{ marginTop: '0.5rem', color: '#ff6b6b', fontWeight: 800, fontSize: '0.88rem' }}>⚠️ {studentError}</div>}
                  </div>
                  {/* Grade Level */}
                  <div style={{ borderRadius: '20px', padding: '1rem 1.2rem', background: isHighlighted('gradeLevel') ? 'rgba(99,179,237,0.15)' : 'rgba(255,255,255,0.04)', border: isHighlighted('gradeLevel') ? '2px solid #63b3ed' : '1.5px solid rgba(255,255,255,0.08)', boxShadow: isHighlighted('gradeLevel') ? '0 0 30px rgba(99,179,237,0.45)' : 'none', transition: 'all 0.3s ease', animation: isHighlighted('gradeLevel') ? 'glowPulse 1.5s ease-in-out infinite' : 'none' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.7rem' }}>
                      {['Kinder', 'Grade 1'].map((grade) => {
                        ...
                        return (<button key={grade} onClick={() => setGradeLevel(grade)} style={{ flex: 1, padding: '0.8rem 0.5rem', borderRadius: '16px', border: isActive ? '2px solid rgba(255,255,255,0.35)' : '1.5px solid rgba(255,255,255,0.1)', cursor: 'pointer', fontFamily: "'Fredoka One', cursive", fontSize: '1rem', background: isActive ? 'linear-gradient(135deg,#11998e,#38ef7d)' : 'rgba(255,255,255,0.06)', color: 'white', boxShadow: isActive ? '0 6px 22px rgba(56,239,125,0.4),0 0 0 2px rgba(56,239,125,0.2)' : 'none', transform: isActive ? 'scale(1.05) translateY(-2px)' : 'scale(1)', transition: 'all 0.25s cubic-bezier(.34,1.56,.64,1)' }}>{grade === 'Kinder' ? '🌱 Kinder' : '📗 Grade 1'}</button>);
                      })}
                    </div>
                  </div>
                  {/* Section */}
                  <div style={{ borderRadius: '20px', padding: '1rem 1.2rem', background: isHighlighted('section') ? 'rgba(183,148,244,0.15)' : 'rgba(255,255,255,0.04)', border: isHighlighted('section') ? '2px solid #b794f4' : '1.5px solid rgba(255,255,255,0.08)', boxShadow: isHighlighted('section') ? '0 0 30px rgba(183,148,244,0.45)' : 'none', transition: 'all 0.3s ease', animation: isHighlighted('section') ? 'glowPulse 1.5s ease-in-out infinite' : 'none' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.6rem' }}>
                      {['1','2','3','4'].map((sec) => {
                        ...
                        return (<button key={sec} onClick={() => setSection(sec)} style={{ flex: 1, padding: '0.85rem 0.4rem', borderRadius: '16px', border: isActive ? '2px solid rgba(255,255,255,0.35)' : '1.5px solid rgba(255,255,255,0.1)', cursor: 'pointer', fontFamily: "'Fredoka One', cursive", fontSize: '1.3rem', background: isActive ? `linear-gradient(${sectionColors[parseInt(sec)-1]})` : 'rgba(255,255,255,0.06)', color: 'white', boxShadow: isActive ? '0 6px 20px rgba(0,0,0,0.3),0 0 0 2px rgba(255,255,255,0.15)' : 'none', transform: isActive ? 'scale(1.1) translateY(-3px)' : 'scale(1)', transition: 'all 0.25s cubic-bezier(.34,1.56,.64,1)' }}>{sec}</button>);
                      })}
                    </div>
                  </div>
                  {/* Login button */}
                  <button onClick={handleStudentSubmit} style={{ padding: '1.15rem', borderRadius: '20px', border: isHighlighted('login') ? '3px solid white' : '2px solid rgba(255,150,0,0.4)', cursor: 'pointer', fontFamily: "'Fredoka One', cursive", fontSize: '1.4rem', color: 'white', background: 'linear-gradient(135deg,#f7971e 0%,#ff5f6d 100%)', boxShadow: isHighlighted('login') ? '0 0 50px rgba(255,200,0,0.9),0 10px 30px rgba(247,151,30,0.6)' : '0 8px 28px rgba(247,151,30,0.45)', transition: 'all 0.3s cubic-bezier(.34,1.56,.64,1)', animation: isHighlighted('login') ? 'glowPulse 1s ease-in-out infinite' : 'none', letterSpacing: '1px', marginTop: '0.25rem' }}
                    onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.05) translateY(-3px)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; }}
              ...
              {activeTab === 'Teacher' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  {[{ label: '👤 Username', id: 'username', type: 'text', val: username, set: setUsername },{ label: '🔒 Password', id: 'password', type: 'password', val: password, set: setPassword }].map(({ label, id, type, val, set }) => (
                    <div key={id} style={{ borderRadius: '20px', padding: '1rem 1.2rem', background: 'rgba(255,255,255,0.04)', border: '1.5px solid rgba(255,255,255,0.08)' }}>
                      ...
                      <input id={id} type={type} value={val} onChange={(e) => set(e.target.value)} style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: '14px', border: '1.5px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.07)', color: 'white', fontSize: '1rem', fontFamily: "'Nunito', sans-serif", fontWeight: 700, outline: 'none' }} />
                    </div>
                  ))}
                  <button onClick={handleTeacherSubmit} disabled={loading} style={{ padding: '1.15rem', borderRadius: '20px', border: '2px solid rgba(167,139,250,0.4)', cursor: 'pointer', fontFamily: "'Fredoka One', cursive", fontSize: '1.4rem', color: 'white', background: 'linear-gradient(135deg,#667eea,#764ba2)', boxShadow: '0 8px 28px rgba(102,126,234,0.45)', transition: 'all 0.3s cubic-bezier(.34,1.56,.64,1)', opacity: loading ? 0.7 : 1, marginTop: '0.25rem' }}>
