import React, { useState, useEffect, useRef, useCallback } from "react";
import { useNavigate } from "react-router-dom";

// ─── Speech Helper ────────────────────────────────────────────────────────────
function useSpeech() {
  const speak = useCallback((text, { onStart, onBoundary, onEnd, pitch = 1.4, rate = 0.92 } = {}) => {
    window.speechSynthesis.cancel();
    const go = () => {
      const utter = new SpeechSynthesisUtterance(text);
      utter.pitch = pitch;
      utter.rate = rate;
      const voices = window.speechSynthesis.getVoices();
      const voice =
        voices.find((v) => /(Google UK English Female|Microsoft Zira|Samantha)/i.test(v.name)) ||
        voices.find((v) => /(female|child|girl|kid|young)/i.test(v.name)) ||
        voices.find((v) => v.lang.startsWith("en")) ||
        voices[0];
      if (voice) utter.voice = voice;
      utter.onstart = () => onStart?.();
      utter.onboundary = (e) => onBoundary?.(e);
      utter.onend = () => onEnd?.();
      utter.onerror = () => onEnd?.();
      window.speechSynthesis.speak(utter);
    };
    const voices = window.speechSynthesis.getVoices();
    if (voices.length === 0) {
      window.speechSynthesis.onvoiceschanged = () => { window.speechSynthesis.onvoiceschanged = null; setTimeout(go, 80); };
    } else { setTimeout(go, 80); }
  }, []);
  const cancel = useCallback(() => { window.speechSynthesis.cancel(); }, []);
  return { speak, cancel };
}

// ─── Confetti Particle ────────────────────────────────────────────────────────
function ConfettiPiece({ color, x, delay, shape }) {
  const style = {
    position: "fixed",
    left: `${x}%`,
    top: "-20px",
    width: shape === "circle" ? "12px" : "10px",
    height: shape === "circle" ? "12px" : shape === "star" ? "10px" : "16px",
    background: color,
    borderRadius: shape === "circle" ? "50%" : shape === "square" ? "2px" : "0",
    animation: `confettiFall ${1.8 + Math.random() * 1.5}s ${delay}s ease-in forwards`,
    zIndex: 9999,
    transform: `rotate(${Math.random() * 360}deg)`,
    clipPath: shape === "star" ? "polygon(50% 0%, 61% 35%, 98% 35%, 68% 57%, 79% 91%, 50% 70%, 21% 91%, 32% 57%, 2% 35%, 39% 35%)" : "none",
  };
  return <div style={style} />;
}

function Confetti({ active }) {
  const pieces = Array.from({ length: 60 }, (_, i) => ({
    id: i,
    color: ["#ff6b6b","#ffd93d","#6bcb77","#4d96ff","#ff922b","#f06","#a29bfe","#fd79a8"][i % 8],
    x: Math.random() * 100,
    delay: Math.random() * 0.8,
    shape: ["circle","square","star"][i % 3],
  }));
  if (!active) return null;
  return <>{pieces.map(p => <ConfettiPiece key={p.id} {...p} />)}</>;
}

// ─── Star Rating Display ──────────────────────────────────────────────────────
function StarBurst({ count = 3 }) {
  return (
    <div style={{ display: "flex", gap: "6px", justifyContent: "center", marginTop: "4px" }}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} style={{
          fontSize: "2rem",
          animation: `starPop 0.4s ${i * 0.15}s both cubic-bezier(0.34,1.56,0.64,1)`,
        }}>⭐</div>
      ))}
    </div>
  );
}

// ─── Floating Emoji Rewards ───────────────────────────────────────────────────
function FloatingReward({ emoji, id, onDone }) {
  useEffect(() => { const t = setTimeout(onDone, 1500); return () => clearTimeout(t); }, []);
  return (
    <div style={{
      position: "fixed",
      left: `${20 + Math.random() * 60}%`,
      bottom: "30%",
      fontSize: "2.5rem",
      animation: "floatReward 1.5s ease-out forwards",
      zIndex: 9998,
      pointerEvents: "none",
    }}>{emoji}</div>
  );
}

// ─── BiboAI Character ─────────────────────────────────────────────────────────
function BiboCharacter({ isSpeaking, isWaving, isHappy, size = 180 }) {
  const [mouthOpen, setMouthOpen] = useState(false);
  const [blinkState, setBlinkState] = useState(false);
  const [sparkles, setSparkles] = useState([]);
  const mouthRef = useRef(null);
  const blinkRef = useRef(null);
  const scale = size / 180;

  useEffect(() => {
    clearInterval(mouthRef.current);
    if (isSpeaking) {
      setMouthOpen(true);
      mouthRef.current = setInterval(() => setMouthOpen((p) => !p), 150);
    } else setMouthOpen(false);
    return () => clearInterval(mouthRef.current);
  }, [isSpeaking]);

  useEffect(() => {
    const scheduleBlink = () => {
      blinkRef.current = setTimeout(() => {
        setBlinkState(true);
        setTimeout(() => { setBlinkState(false); scheduleBlink(); }, 130);
      }, 2200 + Math.random() * 2800);
    };
    scheduleBlink();
    return () => clearTimeout(blinkRef.current);
  }, []);

  useEffect(() => {
    if (isHappy) {
      const newSparkles = Array.from({ length: 6 }, (_, i) => ({
        id: Date.now() + i,
        angle: (i / 6) * 360,
        emoji: ["✨","⭐","💫","🌟","✨","💥"][i],
      }));
      setSparkles(newSparkles);
      setTimeout(() => setSparkles([]), 1200);
    }
  }, [isHappy]);

  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      {sparkles.map(s => (
        <div key={s.id} style={{
          position: "absolute",
          top: "50%", left: "50%",
          fontSize: "1.4rem",
          animation: "sparkleOut 1.2s ease-out forwards",
          transformOrigin: "0 0",
          transform: `rotate(${s.angle}deg) translateX(${80 * scale}px)`,
          zIndex: 10,
          pointerEvents: "none",
        }}>{s.emoji}</div>
      ))}
      <div style={{
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        animation: isWaving ? "bounceUp 0.6s ease" : "floatUpDown 3.5s ease-in-out infinite",
      }}>
        <svg width={180 * scale} height={200 * scale} viewBox="0 0 180 200" fill="none" xmlns="http://www.w3.org/2000/svg"
          style={{ filter: "drop-shadow(0 12px 32px rgba(99,102,241,0.35))" }}>
          {/* Body glow */}
          <ellipse cx="90" cy="165" rx="42" ry="30" fill="url(#bodyGrad)" opacity="0.9" />
          {/* Arms */}
          <ellipse cx="48" cy="152" rx="15" ry="9" fill="#a5b4fc" transform="rotate(-30 48 152)"
            style={{ transformOrigin: "62px 145px", animation: "armSway 3s ease-in-out infinite" }} />
          <ellipse cx="132" cy="148" rx="15" ry="9" fill="#a5b4fc" transform="rotate(30 132 148)"
            style={{
              transformOrigin: "118px 145px",
              animation: isWaving ? "waveArm 0.35s ease-in-out 5 alternate" : "armSway 3s ease-in-out infinite reverse",
            }} />
          {/* Main face circle */}
          <circle cx="90" cy="105" r="60" fill="url(#faceGrad)" />
          <circle cx="90" cy="105" r="60" fill="white" opacity="0.06" />
          {/* Cheeks */}
          <ellipse cx="53" cy="120" rx="13" ry="8" fill="#fda4af" opacity={isHappy ? 0.75 : 0.45} />
          <ellipse cx="127" cy="120" rx="13" ry="8" fill="#fda4af" opacity={isHappy ? 0.75 : 0.45} />
          {/* Eyes */}
          <ellipse cx="70" cy="100" rx="11" ry={blinkState ? 2 : isHappy ? 8 : 12} fill="#1e1b4b" style={{ transition: "ry 0.07s" }} />
          {!blinkState && <circle cx="73.5" cy="95" r="4" fill="white" />}
          <ellipse cx="112" cy="100" rx="11" ry={blinkState ? 2 : isHappy ? 8 : 12} fill="#1e1b4b" style={{ transition: "ry 0.07s" }} />
          {!blinkState && <circle cx="115.5" cy="95" r="4" fill="white" />}
          {/* Happy eye sparkle */}
          {isHappy && !blinkState && <>
            <circle cx="68" cy="107" r="2" fill="white" opacity="0.8" />
            <circle cx="110" cy="107" r="2" fill="white" opacity="0.8" />
          </>}
          {/* Antenna */}
          <line x1="90" y1="47" x2="90" y2="22" stroke="#a5b4fc" strokeWidth="4.5" strokeLinecap="round" />
          <circle cx="90" cy="16" r="10" fill="url(#antennaBall)" />
          <circle cx="90" cy="16" r="5" fill="white" opacity="0.6" />
          {/* Mouth */}
          {mouthOpen ? (
            <>
              <ellipse cx="90" cy="126" rx="17" ry="11" fill="#1e1b4b" />
              <ellipse cx="90" cy="133" rx="9" ry="6" fill="#f87171" />
              <circle cx="80" cy="121" r="3.5" fill="white" />
              <circle cx="100" cy="121" r="3.5" fill="white" />
            </>
          ) : isHappy ? (
            <path d="M70 119 Q90 140 110 119" stroke="#1e1b4b" strokeWidth="4" strokeLinecap="round" fill="none" />
          ) : (
            <path d="M74 122 Q90 136 106 122" stroke="#1e1b4b" strokeWidth="3.5" strokeLinecap="round" fill="none" />
          )}
          {/* Shine */}
          <ellipse cx="70" cy="70" rx="20" ry="13" fill="white" opacity="0.15" transform="rotate(-20 70 70)" />
          {/* Defs */}
          <defs>
            <radialGradient id="bodyGrad" cx="50%" cy="40%">
              <stop offset="0%" stopColor="#818cf8" />
              <stop offset="100%" stopColor="#4f46e5" />
            </radialGradient>
            <radialGradient id="faceGrad" cx="40%" cy="35%">
              <stop offset="0%" stopColor="#ede9fe" />
              <stop offset="100%" stopColor="#c7d2fe" />
            </radialGradient>
            <radialGradient id="antennaBall" cx="40%" cy="35%">
              <stop offset="0%" stopColor="#f9a8d4" />
              <stop offset="100%" stopColor="#ec4899" />
            </radialGradient>
          </defs>
        </svg>
      </div>
      <style>{`
        @keyframes waveArm { 0% { transform: rotate(0deg); } 100% { transform: rotate(-45deg); } }
        @keyframes armSway { 0%, 100% { transform: rotate(0deg); } 50% { transform: rotate(10deg); } }
        @keyframes bounceUp { 0%, 100% { transform: translateY(0) scale(1); } 40% { transform: translateY(-28px) scale(1.05); } 70% { transform: translateY(-10px) scale(1.02); } }
        @keyframes floatUpDown { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-14px); } }
        @keyframes sparkleOut { 0% { opacity: 1; transform: rotate(var(--a)) translateX(0px) scale(0.5); } 100% { opacity: 0; transform: rotate(var(--a)) translateX(90px) scale(1.5); } }
      `}</style>
    </div>
  );
}

// ─── XP Bar ───────────────────────────────────────────────────────────────────
function XPBar({ xp, maxXp = 100 }) {
  const pct = Math.min(100, (xp / maxXp) * 100);
  return (
    <div style={{ width: "100%", maxWidth: 280, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", fontFamily: "'Fredoka One', cursive", color: "#7c3aed", marginBottom: 4 }}>
        <span>⚡ XP</span><span>{xp}/{maxXp}</span>
      </div>
      <div style={{ height: 18, background: "rgba(167,139,250,0.2)", borderRadius: 12, overflow: "hidden", border: "2px solid #c4b5fd" }}>
        <div style={{
          height: "100%", width: `${pct}%`,
          background: "linear-gradient(90deg, #818cf8, #a78bfa, #f472b6)",
          borderRadius: 12,
          transition: "width 0.6s cubic-bezier(0.34,1.56,0.64,1)",
          boxShadow: "0 0 12px rgba(167,139,250,0.6)",
        }} />
      </div>
    </div>
  );
}

// ─── Badge ────────────────────────────────────────────────────────────────────
function Badge({ emoji, label, unlocked }) {
  return (
    <div style={{
      display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
      opacity: unlocked ? 1 : 0.3,
      filter: unlocked ? "none" : "grayscale(100%)",
      transition: "all 0.5s",
      animation: unlocked ? "badgePop 0.5s cubic-bezier(0.34,1.56,0.64,1)" : "none",
    }}>
      <div style={{ fontSize: "2rem", lineHeight: 1 }}>{emoji}</div>
      <div style={{ fontSize: "0.6rem", fontFamily: "'Fredoka One', cursive", color: "#7c3aed", textAlign: "center", maxWidth: 60 }}>{label}</div>
    </div>
  );
}

// ─── Animated Background Stars ────────────────────────────────────────────────
function BackgroundStars() {
  const stars = Array.from({ length: 24 }, (_, i) => ({
    id: i, x: Math.random() * 100, y: Math.random() * 100,
    size: 0.5 + Math.random() * 1.2,
    delay: Math.random() * 4,
    duration: 2 + Math.random() * 3,
  }));
  return (
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
      {stars.map(s => (
        <div key={s.id} style={{
          position: "absolute", left: `${s.x}%`, top: `${s.y}%`,
          width: `${s.size}rem`, height: `${s.size}rem`,
          borderRadius: "50%",
          background: "radial-gradient(circle, rgba(255,255,255,0.9), rgba(196,181,253,0.4))",
          animation: `twinkle ${s.duration}s ${s.delay}s ease-in-out infinite`,
          boxShadow: "0 0 8px rgba(196,181,253,0.8)",
        }} />
      ))}
    </div>
  );
}

// ─── Main Modal ───────────────────────────────────────────────────────────────
const MODAL_TEXT = "Woohoo! I'm BiboAI, your super smart buddy! I can listen and understand everything you say! Let's go on an amazing adventure and learn together!";
const MODAL_WORDS = MODAL_TEXT.split(" ");

// ─── Main Page ────────────────────────────────────────────────────────────────
export default function HiClick() {
  const [phase, setPhase] = useState("idle"); // idle | countdown | intro | done
  const [countdown, setCountdown] = useState(3);
  const [highlightIndex, setHighlightIndex] = useState(-1);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [isWaving, setIsWaving] = useState(false);
  const [isHappy, setIsHappy] = useState(false);
  const [xp, setXp] = useState(0);
  const [showConfetti, setShowConfetti] = useState(false);
  const [rewards, setRewards] = useState([]);
  const [badges, setBadges] = useState({ greeter: false, listener: false, explorer: false });
  const [showStars, setShowStars] = useState(false);
  const [buttonPulse, setButtonPulse] = useState(true);
  const navigate = useNavigate();
  const { speak, cancel } = useSpeech();
  const rewardId = useRef(0);

  const addReward = useCallback((emoji) => {
    const id = rewardId.current++;
    setRewards(r => [...r, { id, emoji }]);
  }, []);

  const triggerWave = useCallback(() => {
    setIsWaving(true);
    setTimeout(() => setIsWaving(false), 1400);
  }, []);

  const triggerHappy = useCallback(() => {
    setIsHappy(true);
    setTimeout(() => setIsHappy(false), 1200);
  }, []);

  useEffect(() => {
    triggerWave();
    speak("Hi there! I'm so excited to see you! Tap the big button to start our adventure!", {
      rate: 0.72,
      onStart: () => setIsSpeaking(true),
      onEnd: () => setIsSpeaking(false),
    });
    return () => cancel();
  }, []);

  const handleStart = () => {
    if (phase !== "idle") return;
    cancel();
    setPhase("countdown");
    setButtonPulse(false);
    triggerHappy();
    addReward("🎉");
    setXp(10);

    speak("Get ready! Three! Two! One! Go!", {
      rate: 0.72,
      onStart: () => setIsSpeaking(true),
      onEnd: () => setIsSpeaking(false),
    });

    let count = 3;
    setCountdown(count);
    const iv = setInterval(() => {
      count--;
      if (count <= 0) {
        clearInterval(iv);
        setPhase("intro");
        startIntro();
      } else {
        setCountdown(count);
      }
    }, 1000);
  };

  const startIntro = () => {
    triggerWave();
    triggerHappy();
    setShowConfetti(true);
    setTimeout(() => setShowConfetti(false), 3000);
    addReward("⭐");
    setXp(40);
    setBadges(b => ({ ...b, greeter: true }));

    speak(MODAL_TEXT, {
      pitch: 1.45, rate: 0.72,
      onStart: () => setIsSpeaking(true),
      onBoundary: (e) => {
        if (e.charIndex !== undefined) {
          const wordIndex = MODAL_TEXT.slice(0, e.charIndex).split(" ").length - 1;
          setHighlightIndex(wordIndex);
        }
      },
      onEnd: () => {
        setIsSpeaking(false);
        setHighlightIndex(-1);
        setXp(100);
        setBadges(b => ({ ...b, listener: true, explorer: true }));
        setShowStars(true);
        triggerHappy();
        addReward("🏆");
        setTimeout(() => {
          setPhase("done");
          navigate("/gameclick");
        }, 2000);
      },
    });
  };

  return (
    <>
      <link href="https://fonts.googleapis.com/css2?family=Fredoka+One&family=Nunito:wght@400;600;700;800&display=swap" rel="stylesheet" />
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { overflow: hidden; }

        @keyframes confettiFall {
          0% { transform: translateY(0) rotate(0deg); opacity: 1; }
          100% { transform: translateY(110vh) rotate(720deg); opacity: 0; }
        }
        @keyframes floatReward {
          0% { transform: translateY(0) scale(0.5); opacity: 1; }
          70% { opacity: 1; }
          100% { transform: translateY(-180px) scale(1.3); opacity: 0; }
        }
        @keyframes twinkle {
          0%, 100% { opacity: 0.2; transform: scale(0.8); }
          50% { opacity: 1; transform: scale(1.2); }
        }
        @keyframes starPop {
          0% { transform: scale(0) rotate(-10deg); opacity: 0; }
          70% { transform: scale(1.3) rotate(10deg); opacity: 1; }
          100% { transform: scale(1) rotate(0deg); opacity: 1; }
        }
        @keyframes badgePop {
          0% { transform: scale(0); }
          70% { transform: scale(1.2); }
          100% { transform: scale(1); }
        }
        @keyframes pulseGlow {
          0%, 100% { box-shadow: 0 0 20px 4px rgba(167,139,250,0.5), 0 8px 32px rgba(99,102,241,0.35); transform: scale(1); }
          50% { box-shadow: 0 0 40px 12px rgba(244,114,182,0.6), 0 12px 40px rgba(99,102,241,0.5); transform: scale(1.04); }
        }
        @keyframes countdownPop {
          0% { transform: scale(0.3); opacity: 0; }
          60% { transform: scale(1.3); opacity: 1; }
          100% { transform: scale(1); opacity: 1; }
        }
        @keyframes shimmer {
          0% { background-position: -200% center; }
          100% { background-position: 200% center; }
        }
        @keyframes wordHighlight {
          0% { transform: scale(1); }
          50% { transform: scale(1.12); }
          100% { transform: scale(1); }
        }
      `}</style>

      <div style={{
        minHeight: "100vh", width: "100vw",
        background: "linear-gradient(135deg, #1e1b4b 0%, #312e81 30%, #4c1d95 60%, #6d28d9 100%)",
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        fontFamily: "'Nunito', sans-serif",
        position: "relative", overflow: "hidden",
        gap: "1.2rem",
      }}>
        <BackgroundStars />
        <Confetti active={showConfetti} />

        {rewards.map(r => (
          <FloatingReward key={r.id} emoji={r.emoji} id={r.id} onDone={() => setRewards(rs => rs.filter(x => x.id !== r.id))} />
        ))}

        {/* Top HUD */}
        <div style={{
          position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)",
          width: "90%", maxWidth: 360,
          background: "rgba(255,255,255,0.08)", backdropFilter: "blur(12px)",
          border: "1.5px solid rgba(196,181,253,0.3)",
          borderRadius: 20, padding: "12px 20px",
          zIndex: 100,
        }}>
          <XPBar xp={xp} />
        </div>

        {/* Badge row */}
        <div style={{
          position: "fixed", top: 88, left: "50%", transform: "translateX(-50%)",
          display: "flex", gap: "1.5rem", zIndex: 100,
        }}>
          <Badge emoji="👋" label="Greeter" unlocked={badges.greeter} />
          <Badge emoji="👂" label="Listener" unlocked={badges.listener} />
          <Badge emoji="🚀" label="Explorer" unlocked={badges.explorer} />
        </div>

        {/* Character */}
        <div style={{ position: "relative", zIndex: 1, marginTop: "4rem" }}>
          <BiboCharacter isSpeaking={isSpeaking} isWaving={isWaving} isHappy={isHappy} size={200} />
        </div>

        {/* IDLE phase */}
        {phase === "idle" && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem", zIndex: 1 }}>
            <h1 style={{
              fontFamily: "'Fredoka One', cursive",
              fontSize: "clamp(2rem, 6vw, 3rem)",
              background: "linear-gradient(90deg, #f9a8d4, #fde68a, #a5f3fc, #c4b5fd)",
              backgroundSize: "200% auto",
              WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
              backgroundClip: "text",
              animation: "shimmer 3s linear infinite",
              textAlign: "center", lineHeight: 1.2,
              textShadow: "none",
            }}>
              Hi there, Superstar! 🌟
            </h1>
            <p style={{ color: "rgba(196,181,253,0.9)", fontSize: "1rem", fontWeight: 600, textAlign: "center" }}>
              Your adventure is waiting...
            </p>

            {/* Speech bubble pointing down at button */}
            <div style={{ position: "relative", display: "flex", justifyContent: "center" }}>
              <div style={{
                background: "white", borderRadius: "16px",
                padding: "8px 18px", fontSize: "0.9rem", fontWeight: 800,
                color: "#4c1d95", boxShadow: "0 4px 20px rgba(0,0,0,0.18)",
                whiteSpace: "nowrap", animation: "twinkle 2s ease-in-out infinite",
              }}>
                Tap the button! 👇
              </div>
            </div>

            <button
              onClick={handleStart}
              style={{
                background: "linear-gradient(135deg, #f472b6, #a78bfa, #60a5fa)",
                border: "none", borderRadius: "50px",
                padding: "18px 52px",
                fontSize: "1.4rem", fontFamily: "'Fredoka One', cursive",
                color: "white", cursor: "pointer",
                animation: buttonPulse ? "pulseGlow 1.8s ease-in-out infinite" : "none",
                boxShadow: "0 8px 32px rgba(167,139,250,0.5)",
                letterSpacing: "0.03em",
                transition: "transform 0.1s",
                position: "relative", overflow: "hidden",
              }}
              onMouseDown={e => e.currentTarget.style.transform = "scale(0.96)"}
              onMouseUp={e => e.currentTarget.style.transform = "scale(1)"}
            >
              🚀 Let's Go!
            </button>
          </div>
        )}

        {/* COUNTDOWN phase */}
        {phase === "countdown" && (
          <div style={{
            fontSize: "clamp(5rem, 20vw, 9rem)",
            fontFamily: "'Fredoka One', cursive",
            color: "#fde68a",
            textShadow: "0 0 60px rgba(253,230,138,0.8), 0 4px 20px rgba(0,0,0,0.4)",
            animation: "countdownPop 0.6s ease-out both",
            key: countdown,
            zIndex: 1,
          }}>
            {countdown === 1 ? "🚀" : countdown}
          </div>
        )}

        {/* INTRO phase: speech bubble text */}
        {phase === "intro" && (
          <div style={{
            background: "rgba(255,255,255,0.07)", backdropFilter: "blur(16px)",
            border: "2px solid rgba(196,181,253,0.4)",
            borderRadius: 24, padding: "24px 28px",
            maxWidth: 420, width: "90%",
            zIndex: 1,
          }}>
            {/* BiboAI label */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              <div style={{
                background: "linear-gradient(135deg, #818cf8, #ec4899)",
                borderRadius: 20, padding: "3px 12px",
                fontSize: "0.8rem", fontFamily: "'Fredoka One', cursive",
                color: "white", letterSpacing: "0.05em",
              }}>🤖 BiboAI</div>
              {isSpeaking && (
                <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
                  {[0, 1, 2].map(i => (
                    <div key={i} style={{
                      width: 5, height: 5, borderRadius: "50%",
                      background: "#a5f3fc",
                      animation: `twinkle 0.6s ${i * 0.15}s ease-in-out infinite`,
                    }} />
                  ))}
                </div>
              )}
            </div>
            {/* Word-highlighted text */}
            <div style={{ fontSize: "1.1rem", lineHeight: 1.9, color: "rgba(255,255,255,0.9)", fontWeight: 600 }}>
              {MODAL_WORDS.map((word, i) => (
                <span key={i} style={{
                  display: "inline-block",
                  background: i === highlightIndex ? "linear-gradient(135deg,#fde68a,#f9a8d4)" : "transparent",
                  color: i === highlightIndex ? "#1e1b4b" : "inherit",
                  borderRadius: 6, padding: "0 3px",
                  transform: i === highlightIndex ? "scale(1.12)" : "scale(1)",
                  transition: "all 0.12s ease",
                  fontWeight: i === highlightIndex ? 800 : 600,
                  marginRight: 3,
                }}>
                  {word}
                </span>
              ))}
            </div>
            {showStars && (
              <div style={{ marginTop: 16 }}>
                <StarBurst count={3} />
                <p style={{ textAlign: "center", color: "#fde68a", fontFamily: "'Fredoka One', cursive", fontSize: "1.1rem", marginTop: 8 }}>
                  Amazing! You earned 3 stars! ⭐
                </p>
              </div>
            )}
          </div>
        )}

        {phase === "done" && (
          <div style={{
            fontSize: "1.5rem", fontFamily: "'Fredoka One', cursive",
            color: "#fde68a", textAlign: "center",
            animation: "countdownPop 0.5s ease-out both",
          }}>
            🎮 Loading your game...
          </div>
        )}
      </div>
    </>
  );
}