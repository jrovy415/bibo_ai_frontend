...
const NextPage = () => {
  ...
  const handleQuestionClick = () => {
    speakText("Let's practice reading! Click the letter for a hint, then click this button if you're ready to speak!");
    setTimeout(() => {
  ...
  const startCountdown = () => {
    ...
    countdownRef.current = setInterval(() => {
      if (count > 0) {
        ...
        setCountdown(`Say ${letters[currentIndex].alt}`);
        speakText(`Say ${letters[currentIndex].alt}`);
        count -= 1;
      } else {
        ...
        setTimeout(() => {
          if (status === 'recording') {
            // Simulate a real click on the Speak Button to stop recording
            const speakButtonElement = speakButtonRef.current?.querySelector('img[alt="Speak Button"]');
            if (speakButtonElement) {
              speakButtonElement.click();
            } else {
              stopRecording();
            }
          }
  ...
  const handleTextClick = (letter) => {
    speakText(letter.alt);
    if (letter.category === 'wiggle') {
      handleWiggle(letter.alt);
    } else if (letter.category === 'glow') {
      handleGlow(letter.alt);
    } else if (letter.category === 'pulse') {
      handlePulse(letter.alt);
    }
  };
