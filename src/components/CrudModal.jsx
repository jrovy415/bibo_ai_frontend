      ...
      return (
        <React.Fragment key={step.key}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: active ? 600 : 400, color: active ? '#7F77DD' : done ? '#1D9E75' : '#aaa', whiteSpace: 'nowrap' }}>
              {step.label}
            </span>
          </div>
...
const WizardInput = (props) => (
  <input {...props} style={{ width: '100%', boxSizing: 'border-box', padding: '9px 12px', borderRadius: 8, border: '1px solid #e0e0e0', fontSize: 14, outline: 'none', fontFamily: 'inherit', background: '#fff', color: '#222', transition: 'border-color 0.2s', ...(props.style || {}) }}
...
const WizardTextarea = (props) => (
  <textarea {...props} style={{ width: '100%', boxSizing: 'border-box', padding: '9px 12px', borderRadius: 8, border: '1px solid #e0e0e0', fontSize: 14, outline: 'none', fontFamily: 'inherit', resize: 'vertical', minHeight: 72, background: '#fff', color: '#222', ...(props.style || {}) }}
    onFocus={e => e.target.style.borderColor = '#7F77DD'}
    onBlur={e => e.target.style.borderColor = '#e0e0e0'}
  />
);
...
const WizardSelect = ({ value, onChange, options, placeholder, disabled }) => (
  <select value={value || ''} onChange={onChange} disabled={disabled} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid #e0e0e0', fontSize: 14, background: '#fff', color: value ? '#222' : '#aaa', outline: 'none', fontFamily: 'inherit', cursor: disabled ? 'not-allowed' : 'pointer' }}>
...
const StepInfo = ({ form, onChange, disabled }) => {
  const difficulties = [
    { value: 'Introduction', label: 'Pre-Test' },
    { value: 'Easy',         label: 'Low Reader' },
    { value: 'Medium',       label: 'Beginning Reader' },
    { value: 'Hard',         label: 'Developing Reader' },
    { value: 'Expert',       label: 'Grade Ready Reader' },
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      ...
      <div>
        ...
        <WizardTextarea value={form.instructions || ''} onChange={e => onChange('instructions', e.target.value)} placeholder="e.g. Read each question carefully." disabled={disabled} rows={3} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          <FieldLabel required>Grade level</FieldLabel>
          <WizardSelect value={form.grade_level} onChange={e => onChange('grade_level', e.target.value)} options={[{ value: 'Kinder', label: 'Kinder' }, { value: 'Grade 1', label: 'Grade 1' }]} placeholder="Pick grade…" disabled={disabled} />
        </div>
        <div>
          <FieldLabel required>Difficulty</FieldLabel>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {difficulties.map(({ value, label }) => {
              const c = DIFFICULTY_COLORS[value];
              const active = form.difficulty === value;
              return (
                <button key={value} disabled={disabled} onClick={() => !disabled && onChange('difficulty', value)} style={{ padding: '7px 4px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit', background: active ? c.bg : '#fafafa', color: active ? c.text : '#bbb', border: `1.5px solid ${active ? c.border : '#e8e8e8'}`, transition: 'all 0.15s' }}>
                  {label}
                </button>
              );
            })}
      ...
      <div>
        ...
        <span style={{ marginLeft: 8, fontSize: 13, color: '#888' }}>{form.is_active ? 'Visible to students' : 'Hidden from students'}</span>
      </div>
    </div>
  );
};
...
const StepMaterial = ({ material, onChange, disabled }) => (
  <div style={{ display: 'grid', gap: 14 }}>
    <div style={{ background: '#f8f6ff', borderRadius: 10, padding: '12px 14px', border: '1px solid #e8e4f8', fontSize: 13, color: '#7F77DD' }}>
...
const StepQuestions = ({ questions, setQuestions, questionTypeOptions, fileList, setFileList, data, mode }) => {
  ...
  return (
    <div>
      ...
      {questions.length === 0 && (
        <div style={{ textAlign: 'center', padding: '32px 20px', borderRadius: 10, background: '#fafafa', border: '1.5px dashed #e0e0e0', color: '#bbb', fontSize: 14, marginBottom: 14 }}>
      ...
      {questions.map((question, qi) => {
        ...
        const useWordScoring  = question.use_word_scoring === true; // default false (Exact Match)

        return (
          <div key={qi} style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, marginBottom: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>

            {/* Question header */}
            <div style={{ background: '#f8f6ff', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid #eeebfb' }}>
            ...
            <div style={{ padding: '14px 16px', display: 'grid', gap: 12 }}>
              ...
              {/* Question text */}
              <div>
                <FieldLabel hint={question.is_answer !== true ? '(optional when answer is separate)' : ''}>Question text</FieldLabel>
                <WizardTextarea value={question.question_text || ''} onChange={e => updateQuestion(qi, 'question_text', e.target.value)} placeholder="Type your question here…" disabled={isDisabled} rows={2} />
              </div>

              {/* Is Answer toggle */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Switch checked={!!question.is_answer} onChange={v => updateQuestion(qi, 'is_answer', v)} disabled={isDisabled} size="small" />
                <span style={{ fontSize: 13, color: '#666' }}>{question.is_answer ? 'The question text is shown to the student' : 'Show a separate answer field below'}</span>
              </div>

              {question.is_answer === false && (
                <div>
                  <FieldLabel required>Actual answer</FieldLabel>
                  <WizardInput value={question.actual_answer || ''} onChange={e => updateQuestion(qi, 'actual_answer', e.target.value)} placeholder="Enter the correct answer" disabled={isDisabled} />
                </div>
              )}
...
const arrowBtn = { background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', fontSize: 15, padding: '0 4px', fontFamily: 'inherit' };
...
const StepReview = ({ quizForm, material, questions, questionTypeOptions }) => {
  ...
  const diffLabels = { Introduction: 'Pre-Test', Easy: 'Low Reader', Medium: 'Beginning Reader', Hard: 'Developing Reader', Expert: 'Grade Ready Reader', PostTest: 'Post-Test' };

  return (
    <div style={{ display: 'grid', gap: 14 }}>
      <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, padding: '14px 16px' }}>
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8, color: '#222' }}>{quizForm.title || <span style={{ color: '#ccc' }}>(no title)</span>}</div>
        {quizForm.instructions && <div style={{ fontSize: 13, color: '#777', marginBottom: 10 }}>{quizForm.instructions}</div>}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          ...
          {quizForm.time_limit && <span style={{ background: '#f5f5f5', color: '#888', borderRadius: 20, padding: '2px 10px', fontSize: 12 }}>⏱ {quizForm.time_limit} min</span>}
      ...
      {(material.title || material.content) && (
        <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, padding: '12px 16px' }}>
          ...
          {material.content && <div style={{ fontSize: 12, color: '#aaa', marginTop: 2 }}>{material.type === 'story' ? `${material.content.slice(0, 80)}${material.content.length > 80 ? '…' : ''}` : material.content}</div>}
        </div>
      )}

      <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, padding: '12px 16px' }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#aaa', marginBottom: 10, textTransform: 'uppercase', letterSpacing: 1 }}>Questions ({questions.length})</div>
        ...
        {questions.map((q, i) => {
          ...
          const typeName      = selectedType?.label;
          const isReading     = typeName === 'Reading';
          const useWordScoring = q.use_word_scoring === true;
          const hasCorrect    = q.is_answer === false ? !!q.actual_answer?.trim() : q.choices?.some(c => c.is_correct);

          return (
            <div key={i} style={{ display: 'flex', gap: 10, paddingBottom: 10, marginBottom: 10, borderBottom: i < questions.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {q.is_answer === false
                    ? <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 5, background: '#E1F5EE', color: '#0F6E56', border: '1px solid #9FE1CB' }}>✓ {q.actual_answer || '(no answer)'}</span>
                    : (q.choices || []).map((c, ci) => (
                        <span key={ci} style={{ fontSize: 11, padding: '2px 7px', borderRadius: 5, background: c.is_correct ? '#E1F5EE' : '#f5f5f5', color: c.is_correct ? '#0F6E56' : '#888', border: `1px solid ${c.is_correct ? '#9FE1CB' : '#e8e8e8'}` }}>
                          {c.is_correct ? '✓ ' : ''}{c.choice_text || '(empty)'}
                        </span>
                      ))
                  }
                </div>
                {!hasCorrect && <div style={{ fontSize: 11, color: '#e74c3c', marginTop: 4 }}>⚠ No correct answer marked</div>}
              </div>
...
const validateQuizStep = (step, quizForm, questions) => {
  ...
  if (step === 2) {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.is_answer === false) { if (!q.actual_answer?.trim()) return `Question ${i + 1}: please enter the actual answer.`; }
      else { if (!q.question_text?.trim()) return `Question ${i + 1}: please enter question text.`; }
    }
  }
  return null;
};
...
const CrudModal = ({ visible, onCancel, mode, title, data, fields, onSubmit, loading = false, authUser, questionTypeOptions = [] }) => {
      ...
      if (isQuiz) {
        ...
        if (data?.questions) {
          setQuestions(data.questions.map(q => {
            const isSep = !q.question_text?.trim() && q.choices?.length === 1 && q.choices[0]?.is_correct === true;
            return { ...q, is_answer: isSep ? false : true, actual_answer: isSep ? (q.choices[0]?.choice_text || '') : '', choices: isSep ? [] : (q.choices || []), use_word_scoring: q.use_word_scoring === true };
          }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isQuiz) {
        ...
        for (let i = 0; i < questions.length; i++) {
          const q = questions[i]; const typeLabel = questionTypeOptions?.find(o => o.value === q.question_type_id)?.label;
          if (q.is_answer === false) { if (!q.actual_answer?.trim()) { message.error(`Question ${i+1}: please enter the actual answer.`); setSubmitLoading(false); return; } }
          else { if (!q.question_text?.trim()) { message.error(`Question ${i+1}: please enter question text.`); setSubmitLoading(false); return; } }
        ...
        const baseData = { ...(mode === 'edit' ? data : {}), ...quizForm, time_limit: quizForm.time_limit ? parseInt(quizForm.time_limit) : null, material: (material.title || material.content) ? material : null };
        ...
        if (hasFileUploads) {
          ...
          Object.keys(baseData).forEach(key => { if (key !== 'questions') { const v = baseData[key]; fd.append(key, typeof v === 'object' && v !== null ? JSON.stringify(v) : v ?? ''); } });
          questions.forEach((q, qi) => {
            ...
            if (q.is_answer === false) { fd.append(`questions[${qi}][choices][0][choice_text]`, q.actual_answer || ''); fd.append(`questions[${qi}][choices][0][is_correct]`, '1'); }
            else { q.choices?.forEach((c, ci) => { fd.append(`questions[${qi}][choices][${ci}][choice_text]`, c.choice_text || ''); fd.append(`questions[${qi}][choices][${ci}][is_correct]`, c.is_correct ? '1' : '0'); if (c.id) fd.append(`questions[${qi}][choices][${ci}][id]`, c.id); }); }
          });
          await onSubmit(fd);
        } else {
      ...
      <div>
        ...
        {title === 'Quiz' && (
          <>
            ...
            ) : <div style={{ marginTop: 24 }}><Divider orientation="left">Material (Optional)</Divider><p style={{ color: '#888', fontStyle: 'italic' }}>No material added</p></div>}
  ...
  return (
    <Modal
      title={<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontWeight: 600, fontSize: 18 }}>{getModalTitle()}</span></div>}
      open={visible} onCancel={onCancel} footer={renderFooter()}
      width={isQuiz ? 720 : 1000} confirmLoading={loading}
      bodyStyle={{ maxHeight: '76vh', overflowY: 'auto', padding: '24px 28px', background: '#fafafa', borderRadius: 8 }}
